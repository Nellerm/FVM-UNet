...
class GT_CeDiceLoss(nn.Module):
    def __init__(self, num_classes, loss_weight=[0.4, 0.6]):
        super(GT_CeDiceLoss, self).__init__()
        self.cedice = CeDiceLoss(num_classes, loss_weight)

    def forward(self, gt_pre, out, target):
        cediceloss = self.cedice(out, target)
        gt_pre4, gt_pre3, gt_pre2, gt_pre1 = gt_pre
        gt_loss = self.cedice(gt_pre4, target) * 0.1 + self.cedice(gt_pre3, target) * 0.2 + self.cedice(gt_pre2, target) * 0.3 + self.cedice(gt_pre1, target) * 0.4
        return cediceloss + gt_loss
...
class GT_BceDiceLoss(nn.Module):
    ...
    def forward(self, gt_pre, out, target):
        ...
        gt_pre4, gt_pre3, gt_pre2, gt_pre1 = gt_pre
        gt_loss = self.bcedice(gt_pre4, target) * 0.1 + self.bcedice(gt_pre3, target) * 0.2 + self.bcedice(gt_pre2, target) * 0.3 + self.bcedice(gt_pre1, target) * 0.4
...
def print_model_parameters(model, output_dir):
    total_params = sum(p.numel() for p in model.parameters())
    total_bytes = total_params * 4  # Assuming float32 (4 bytes per parameter)
    total_megabytes = total_bytes / (1024 ** 2)

    # 创建输出目录（如果不存在）
    os.makedirs(output_dir, exist_ok=True)
    output_file = os.path.join(output_dir, 'model_param.txt')
    ...
    with open(output_file, 'w') as f:
        ...
        # 打印换算后的MB
        f.write(f"Total size of parameters: {total_megabytes:.2f} MB\n\n")

        # 打印每一层的参数量
        f.write("Layer-wise parameter counts:\n")
...
def test_single_volume(image, label, net, classes, patch_size=[256, 256], 
    ...
    if len(image.shape) == 3:
        ...
        for ind in range(image.shape[0]):
            ...
            net.eval()
            with torch.no_grad():
                outputs, _ = net(input) # net_return: out, gt_pre
        ...
        net.eval()
        with torch.no_grad():
            out, _ = net(input)  # net_return: out, gt_pre
